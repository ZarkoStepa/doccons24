...
Invite user into session link
    [Documentation]    create new session
    [Tags]    admin.create.appointments
    Wait Until Element Is Visible    id:m_aside_left_offcanvas_toggle
    Click Element    id:m_aside_left_offcanvas_toggle
    Wait Until Element Is Visible    xpath://span[contains(text(),'Create Session Link')]
    ...
    Click Element    xpath://table[1]/tbody[1]/tr[1]/td[6]/div[1]/div[1]
    Wait Until Element Is Visible    xpath://div[@id='windowModalForSendingMailUsers']//h4[@class='modal-title'][contains(text(),'Invite guest to video')]
    ...
    Wait Until Element Is Visible    xpath://div[@id='windowModalForSendingMailUsers']//label[@class='control-label col-sm-2'][contains(text(),'Enter email')]
    ...
    Input Text    id:email2    ${DOCEMAIL}
    Capture Page Screenshot    input-docmail-{index}.png
    Element Should Be Enabled    id:footer_action_button2
    Element Text Should Be    id:footer_action_button2    Send Invitation to Guest
    Element Should Be Visible    id:footer_action_button2
    Element Should Be Enabled    xpath://div[@id='windowModalForSendingMailUsers']//button[@class='btn btn-danger'][contains(text(),'Close')]
    Element Should Be Visible    xpath://div[@id='windowModalForSendingMailUsers']//button[@class='btn btn-danger'][contains(text(),'Close')]
    Element Text Should Be    xpath://div[@id='windowModalForSendingMailUsers']//button[@class='btn btn-danger'][contains(text(),'Close')]    Close
    Capture Page Screenshot    invite-link-for-sessions-{index}.png
    Wait Until Element Is Visible    id:footer_action_button2
    Click Element    id:footer_action_button2
    Capture Page Screenshot    invite-link-for-sessions-button2-{index}.png
    Sleep    2
...
Doctor check email
    ...
    [Tags]    admin.create.appointments
    GoTo    ${email_server}
    Click Element    id:inbox_field
    Clear Element Text    id:inbox_field
    Wait Until Element Is Visible    id:inbox_field
    Input Text    id:inbox_field    ${DOCUSER}
